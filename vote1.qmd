...
| `vote_dem` | Porcentaje de votos obtenidos por el candidato demócrata (el porcentje que obtuvo el candidato republicano es `100 - vote_dem`) |
...
```{r}
vote$more_exp <- factor(ifelse(vote$exp_dem - vote$exp_rep > 0, 
                               "Dem.", "Rep."), 
                        levels = c("Dem.", "Rep."))
vote$more_votes <- factor(ifelse(vote$vote_dem - 50 > 0,
                                 "Dem. won", "Rep. won"),
                          levels = c("Dem. won", "Rep. won"))
```

Una tabla de contingencia:

```{r}
datasummary_crosstab(more_exp ~ more_votes, data = vote,
